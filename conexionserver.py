...
class ConexionServer():
    ...
    def listadoClientes():
        try:
            ...
            if historico:
                cursor.execute(
                    "SELECT dnicli, altacli, apelcli, nomecli, emailcli, movilcli, dircli, provcli, municli, bajacli  FROM clientes ORDER BY apelcli, nomecli ASC")
                resultados = cursor.fetchall()
                ...
                cursor.execute(
                    "SELECT dnicli, altacli, apelcli, nomecli, emailcli, movilcli, dircli, provcli, municli, bajacli  FROM clientes WHERE bajacli is null or bajacli = '' ORDER BY apelcli, nomecli ASC")
                resultados = cursor.fetchall()
                # Procesar cada fila de los resultados
                for fila in resultados:
                    # Crear una lista con los valores de la fila
    ...
    def cargarAllPropiedadesBD():
        try:
            ...
            cursor = conexion.cursor()
            cursor.execute(
                "SELECT codigo,altaprop,bajaprop,dirprop,provprop,muniprop,tipoprop,habprop,banprop,superprop,prealquiprop,prevenprop,cpprop,obserprop,tipooper,estadoprop,nomeprop,movilprop  FROM propiedades ORDER BY muniprop ASC")
